...
import VNA_functions as VNAf

DATA_LABEL = { 'PLOG': ('logmag','phase'),
              'SCOM': ('real', 'imag'),
                'PLIN': ('magnitude', 'phase'),
                'POL': ('real', 'imag'),
                'MLOG': ('logmag')}
...
def set_rtSwitch_S_SS():
    ...
    set_rtSwitch(switch="B",port="0") #In: 11
    set_rtSwitch(switch="E",port="1")
    set_rtSwitch(switch="F",port="0") #Out: D

def set_rtSwitch_S_II():
    set_rtSwitch(switch="A",port="1")
    set_rtSwitch(switch="B",port="1") #In: 9
    set_rtSwitch(switch="E",port="1") 
    set_rtSwitch(switch="F",port="1") #Out: C
...
#            vna.set_averaging_trigger(False)
            vna.do_enable_averaging(True) #works for pnax, need above for vna
...
# take a VNA power sweep at each flux point
def flux_VNAp_sweep(h5filename, vna, yoko, vna_pows=[-50], curr_start=-2.5e-3, curr_stop=2.5e-3, curr_step=1e-6,
                    if_bandwidth=3000,num_points=1601, avgs_start=100, avgs_stop=2, display=True, atten=0, vna_fmt='SCOM'):
    numCurr = round(abs(curr_stop - curr_start) / curr_step + 1)
    currents = np.linspace(curr_start, curr_stop, numCurr)
    vna.set_points(num_points)
    slew = 10e-6  #yoko slew in [A/s]
    for i_current, current in enumerate(currents):
        ...
        if wait_time is not None:            
            sleep(wait_time + 1) #1 second for GPIB overhead
        data = VNAf.power_sweep(vna,vna_pows,IF_BANDWIDTH=if_bandwidth,NUM_POINTS=num_points,
                                  avgs_start=avgs_start, avgs_stop=avgs_stop, save=False,
                                  display=display, atten=atten, revert=False, fmt=vna_fmt,
                                  h5file=h5filename, extParam_name='current', extParam=current)
        vna.set_power(vna_pows[0])
    return data #only most recent power sweep
                    

# heuristic for guessing resonant frequency, max(real part) works when phase=0 on resonance
def guessF_maxReal(aout_complex):
    # aout_complex: 1D array of complex numbers
    # returns index value in above arrays corresponding to guess of resonant frequency
    aout_real = np.real(aout_complex)
    return np.argmax(aout_real)

# heuristic for guessing resonant freq, min(real part), works when phase=180 on resonance
def guessF_minReal(aout_complex):
    aout_real = np.real(aout_complex)
    return np.argmin(aout_real)

# heuristic for guessing resonant frequency, middle point of sweep
def guessF_middle(aout_complex):
    return aout_complex.shape[0] / 2
    
# takes power sweep data for 1 resonator in reflection and fits it for internal Q and coupling Q
def power_sweep_Qfit(h5filename, dataDir=None, kc=10e6, ki=1e6, display=True, guessF=guessF_maxReal, save=True, numFitPoints=0, extParam_name=None, extParam=0.0):
    ...
    # kc: kappaC/2pi guess in Hz
    # ki: kappaI/2pi guess in Hz
    # display: True displays graphs of all fits from analyze_reflection
    # guessF: heuristic funtion of 1 argument (a_complex) that returns index for guess of resonant frequency
    # save: True to save fit params to h5file under folder 'fits'    
    data = load_powerSweepData(h5filename, dataDir=dataDir, vna_fmt='SCOM')
    freqs = data['frequencies']
    powers = data['VNA_power']
    ...
    aout_complex = data['a_complex']
    #import sys
    #sys.path.insert(0, 'Z:/Data/JPC_2015/2015-08-27 Cooldown')
    f_0_ind = guessF(aout_complex[0,:])
    f_0 = freqs[f_0_ind]
    if numFitPoints != 0:
        i_start = max(f_0_ind-numFitPoints, 0)
        i_stop = min(f_0_ind+numFitPoints, aout_complex.shape[1])
        aout_complex = aout_complex[:,i_start:i_stop]
        freqs = freqs[i_start:i_stop]
    ...
    result = af.analyze_reflection(freqs, powers, aout_complex, f_0=f_0, kc=kc,ki=ki, a_in=a_background, T=1e-12, display=display)
    ...
    if save:
        save_power_Qfit(result, h5filename, dataDir=dataDir, extParam_name=extParam_name, extParam=extParam)
...
def flux_sweep_Qfit(h5filename, dataDir=None, numFitPoints=0, kc=10e6, ki=1e6, display=True, guessF=guessF_maxReal, save=True):
    # h5filename: full file path and name of h5file with data to be fit
    # dataDir: relative directory inside h5file where data is store (eg '/150914/144652'), if None then the most recent data will be used 
    ...
    # kc: kappaC/2pi guess in Hz
    # ki: kappaI/2pi guess in Hz
    # display: True to display all plotted fits from analyze_reflection
    # guessF: heuristic funtion of 1 argument (a_complex) that returns index for guess of resonant frequency
    # save: True to save fit results to same h5file under folder 'fits'
    data = load_fluxData(h5filename, dataDir=dataDir)
    freqs = data['frequencies']
    currents = data['currents']
    aout_complex = data['a_complex']     # assumes data was taken in SCOM form (real and imag)
    (currents,Qc,Qi,Qtot,results) = flux_sweep_Qfit_helper(freqs, currents, aout_complex, numFitPoints=numFitPoints,
                                                            kc=kc, ki=ki, display=display, guessF=guessF)
    if save:
        save_flux_Qfit(results, h5filename, dataDir=dataDir)
    return (currents,Qc,Qi,Qtot,results)
    

# fits flux sweep data for Q
def flux_sweep_Qfit_helper(freqs, currents, aout_complex, numFitPoints=0, kc=10e6, ki=1e6, display=True,
                           guessF=guessF_maxReal, updateKappaGuess=False):
    ...
    for ind_curr, current in enumerate(currents):
        f_0_ind = guessF(aout_complex[ind_curr, :])
        f_0 = freqs[f_0_ind]
        ...
        if numFitPoints != 0:
            i_start = max(f_0_ind-numFitPoints, 0)
            i_stop = min(f_0_ind+numFitPoints, aout_slice.shape[0])
            aout_slice = aout_slice[i_start:i_stop]
            freqs_slice = freqs_slice[i_start:i_stop]
        # a_background takes an average of points off resonance to get a resonable initial guess for a_in
        a_background = (sum(aout_slice[0:2]) + sum(aout_slice[-2:])) / 4
        result = af.analyze_reflection(freqs_slice, current, aout_slice, f_0 = f_0, kc=kc_guess,ki=ki_guess, a_in=a_background, T=1e-12, display=display)
        results[ind_curr,:] = np.array((result[0][0],result[1][0],result[2][0],np.real(result[3][0]), np.imag(result[3][0]), result[4][0]))
        if updateKappaGuess:
            kc_guess = results[ind_curr, 1]
            ki_guess = results[ind_curr, 2]
...
# save Qfit data for a flux sweep in dataDir/'fits'
def save_flux_Qfit(results, h5filename, dataDir=None, extParam_name=None, extParam=0.0):
    ...
    try:
        if dataDir:
            ...
            meas = grp[grp.keys()[-1]]
        g = meas.require_group('fits')
        save_Qfit_helper(results, g, extParam_name=extParam_name, extParam=extParam)
    except Exception as e:
        print "EXCEPTION", e
        raise
    finally:
...
def save_Qfit_helper(results, g, extParam_name=None, extParam=0.0):
    gkeys = g.keys()
    if gkeys:
        g['f0'][:] = results[:,0]
        g['kc'][:] = results[:,1]
        g['ki'][:] = results[:,2]
        g['a_in_real'][:] = results[:,3]
        g['a_in_imag'][:] = results[:,4]
        g['T'][:] = results[:,5]
        if extParam_name:
            g['f0'].attrs.create(extParam_name, extParam)
    else:
        g0 = g.create_dataset('f0', data= results[:,0])
        g1 = g.create_dataset('kc', data= results[:,1])
        g2 = g.create_dataset('ki', data= results[:,2])
        g3 = g.create_dataset('a_in_real', data= results[:,3])
        g4 = g.create_dataset('a_in_imag', data= results[:,4])
        g5 = g.create_dataset('T', data= results[:,5])
        if extParam_name:
            g0.attrs.create(extParam_name, extParam)
  
# save Qfit data for a vna power sweep in dataDir/'fits'
def save_power_Qfit(result, h5filename, dataDir=None, extParam_name=None, extParam=0.0):
    results = np.array((result[0][:],result[1][:],result[2][:],np.real(result[3][:]), np.imag(result[3][:]), result[4][:]))
    results = np.transpose(results)
    save_flux_Qfit(results, h5filename, dataDir=dataDir, extParam_name=extParam_name, extParam=extParam)

# plot Qfit of sweep from h5file data
def plot_flux_Qfit(h5filename, dataDir=None, vna_fmt='SCOM'):
    data = load_sweepData(h5filename, dataDir=dataDir, vna_fmt=vna_fmt, sweep_var='currents')
    fits = data['fits']
    Qc = fits['f0'] / fits['kc']
    Qi = fits['f0'] / fits['ki']
    Qtot = fits['f0'] / (fits['kc']+fits['ki'])
    plot_Qfit(data['currents'], fits['f0'], Qc, Qi, Qtot, xdataType='Current')
    
# plot power sweep Qfit from h5file
def plot_power_Qfit(h5filename, dataDir=None, vna_fmt='SCOM'):
    data = load_sweepData(h5filename, dataDir=dataDir, vna_fmt=vna_fmt, sweep_var='VNA_power')
    fits = data['fits']
    Qc = fits['f0'] / fits['kc']
    Qi = fits['f0'] / fits['ki']
    Qtot = fits['f0'] / (fits['kc']+fits['ki'])
    plot_Qfit(data['VNA_power'], fits['f0'], Qc, Qi, Qtot, xdataType='Power')
...
# load Qfit data stored in dataDir/fits
def load_Qfit(h5filename, dataDir=None, extParam_name=None):
    h5file = h5py.File(h5filename)
    if dataDir:
        meas = h5file[dataDir]
    else:
        ...
        meas = grp[grp.keys()[-1]]
    g = meas['fits']
    fitdata = load_Qfit_helper(g, extParam_name)
    h5file.close()
    return fitdata

def load_Qfit_helper(g, extParam_name=None):
    fitdata = {'f0':np.array(g['f0']),
               'kc':np.array(g['kc']),
                'ki':np.array(g['ki']),
                'a_complex': (np.array(g['a_in_real']) + 1j*np.array(g['a_in_imag'])),
                'T': np.array(g['T'])}
    if extParam_name and extParam_name in g.keys():
        fitdata[extParam_name] = np.array([g['f0'].attrs[extParam_name]])[0]
    return fitdata

def load_fluxData(h5filename, dataDir=None, vna_fmt='SCOM'):
    return load_sweepData(h5filename, dataDir=dataDir, vna_fmt='SCOM', sweep_var='currents')
    
def load_powerSweepData(h5filename, dataDir=None, vna_fmt='SCOM'):
    return load_sweepData(h5filename, dataDir=dataDir, vna_fmt='SCOM', sweep_var='VNA_power')
    
# load data from h5file from a flux sweep or a vna power sweep. Sweep var is either 'currents' or 'VNA_power'    
def load_sweepData(h5filename, dataDir=None, vna_fmt='SCOM', sweep_var = None):
    # h5filename: full file path and name of h5file with data to be fit
    ...
    if dataDir:
        ...
        grp = h5file[h5file.keys()[-1]]
        meas = grp[grp.keys()[-1]]
    a_0 = np.array(meas[DATA_LABEL[vna_fmt][0]])
    a_1 = np.array(meas[DATA_LABEL[vna_fmt][1]])
    if vna_fmt == 'SCOM' or vna_fmt=='POL':
        a_complex = a_0 + 1j*a_1
    elif vna_fmt == 'PLIN':
        a_complex = a_0 * np.exp(np.pi/180.0 *1j*a_1)
    elif vna_fmt == 'PLOG':
        a_complex = PLOG_to_complex(a_0, a_1)
    else:
        print('Error: vna format not supported for load_sweepData')
    data = {'frequencies':np.array(meas['frequencies']),
            'a_complex':a_complex}
    if sweep_var:
        data[sweep_var] = np.array(meas[sweep_var])
    fits = False
    if 'fits' in meas.keys():
        fits = True
    h5file.close()
    if fits:
        data['fits'] = load_Qfit(h5filename, dataDir=dataDir)
    return data

def PLOG_to_complex(logmag, phase):
    return np.exp(logmag/20 + 1j*np.pi/180.0 * phase)

def complex_to_PLOG(a_complex, phase_shift=0, degrees=True):
    logmag = 20*np.log10(np.abs(a_complex))
    phase = np.angle(a_complex * np.exp(1j*np.pi/180.0 * phase_shift), deg=degrees)
    return (logmag, phase)
...
def colorMap_flux(currents, freqs, a_abs, a_phase, colorMap=('hot','phase'), title=None,savepath=None):
...
#  calc P_-1dB from a DR trace: takes an average of first 1/6 points for low power value
def calc_P1dB(vna_pows, dr_trace, display=True):
    num_pows = vna_pows.shape[0]
    i_ave = int(num_pows / 6)
...
def take_2Dpump_sweep(vna, ag, spec=None, pow_start=-20, pow_stop=-15, pow_step= 0.01, averages=(100,50),
                      freq_start=14e9, freq_stop=14e9, freq_step=1e6, num_points=1601, vna_states=('Signal','Idler'),
                        change_rtSwitch=(set_rtSwitch_S_SS, set_rtSwitch_S_II), display=True, atten=0, vna_fmt='PLOG', phase_save=False, h5file=None):
    ...
    for i_freq, freq in enumerate(pump_freqs):
        ...
        (pows, freqs_S, data_S, freqs_I, data_I) = take_Ppow_sweep(vna, ag, spec=spec, pow_start=pow_start, pow_stop=pow_stop, pow_step=pow_step,
                                                                    ...
                                                                    change_rtSwitch=change_rtSwitch, display=display, atten=atten, vna_fmt=vna_fmt,
                                                                    phase_save=phase_save, h5file=h5file)
        data['pows'] = pows
...
def take_Ppow_sweep(vna, ag, spec=None, pow_start=-20, pow_stop=-15, pow_step=0.01, averages=(100,50),
    ...
    try:
        ...
        for i_power, power in enumerate(pows):
            ...
            vna.set_averaging_state(False)
            if spec:
                spec.ask('*OPC?') # waits until spectrum single measurement operation is complete
...
def plot_Ppow_sweep_data(h5filename, dataDir=None, vna_states=('Signal', 'Idler'), nvr=True, skip=1, cutTop=0, memDataDirs=None, memvna_fmt='SCOM', save=None, atten_p=0):
    (vnadata, specdata) = load_Ppow_sweep_data(h5filename, dataDir=dataDir, vna_states=vna_states, nvr=nvr)
    memdataL = []    
    if memDataDirs:
        for memDir in memDataDirs:
            data = load_sweepData(h5filename, memDir, vna_fmt=memvna_fmt)
            memdataL.append(complex_to_PLOG(data['a_complex'])[0])
    colormap = plt.get_cmap('rainbow')
    powers = vnadata['powers'] + atten_p
    powers = powers[:(len(powers)-cutTop):skip]
    num_pows = powers.size
    si = ('S','I')
    figsize = (3,3)
    tick_fontsize = 8 #Draft 1: 10
    label_size = 10 #Draft 1: 12
    xlabelpad = 0.25
    ylabelpad = 2
    figS, axS = plt.subplots(2,1, sharex=nvr, figsize=figsize)
    if nvr:
    ...
    figI, axI = plt.subplots(2,1, sharex=nvr, figsize=figsize)
    if nvr:
    ...
    for k in xrange(2):
        fig = figs[k]
        ax = axSI[k]
        ax[0].set_title(vna_states[k], size=label_size)
        f_g = vnadata['freqs_'+si[k]] * 1e-9
        gain = vnadata['gain_'+si[k]]
        gain = gain[:(len(gain[:,0])-cutTop):skip, :]
        if memDataDirs:
            gain = gain - memdataL[k]
        if nvr:
            f_n = specdata['freqs_'+si[k]] * 1e-9
            nvr_data = specdata['p_on_'+si[k]]
            nvr_data = nvr_data[:(len(nvr_data[:,0])-cutTop):skip, :]
        ...
        ax[0].set_ylim([-1, 25])
        ax[1].set_ylim([-1, 12])
        ax[0].set_yticks(np.arange(6)*5)
        ax[1].set_yticks(np.arange(3)*5)
        plt.setp(ax[0].get_yticklabels()[1::2], visible=False)
        plt.setp(ax[1].get_yticklabels()[1::2], visible=False)
        ax[1].set_xlabel('Frequency (GHz)', size=label_size, labelpad=xlabelpad)
        ax[0].set_ylabel('Gain (dB)', size=label_size, labelpad=ylabelpad)
        ax[1].set_ylabel('NVR (dB)', size=label_size, labelpad=ylabelpad)
        plt.setp(ax[0].get_yticklabels(), fontsize=tick_fontsize)
        plt.setp(ax[1].get_yticklabels(), fontsize=tick_fontsize)
        plt.setp(ax[1].get_xticklabels(), fontsize=tick_fontsize)
#        fig.colorbar()
#    axI[1].set_xlim([7.65,7.74])
    axI[1].set_xlim([4.98, 5.02])
    axS[1].set_xlim([9.182, 9.222])
    axI[1].set_xticks([4.98, 4.99, 5.0, 5.01, 5.02])
    axS[1].set_xticks([9.19, 9.20, 9.21, 9.22])
#    plt.setp(axI[1].get_xticklabels(), fontsize=tick_fontsize)
#    plt.setp(axS[1].get_xticklabels(), fontsize=tick_fontsize)
    plt.setp(axI[1].get_xticklabels()[1::2], visible=False, fontsize=tick_fontsize)
    plt.setp(axS[1].get_xticklabels()[1::2], visible=False, fontsize=tick_fontsize)
#    axI[1].set_xticks([7.66, 7.68, 7.70, 7.72, 7.74])
    cfig, cax = plt.subplots(figsize=(0.5,3))
    norm = plt_colors.Normalize(vmin=powers[0], vmax=powers[-1])
    cb = plt_colorbar.ColorbarBase(cax, cmap=colormap, norm=norm, ticks=[-55,-56,-57,-58,-59,-60])
    cb.set_label('Pump power (dBm)', rotation=90, size=label_size, labelpad=ylabelpad+2)
    cax.tick_params(width=width, labelsize=tick_fontsize)
    for axis in ['top', 'bottom','left','right']:
...
#            figs[k].set_size_inches(8,6)
            ...
            figs[k].savefig(save + r'\fig_gainNVR_' + si[k])
#        cfig.set_size_inches(0.5,2)
        cfig.subplots_adjust(right=0.25)
        cfig.subplots_adjust(bottom=0.15) #new Draft 2
        cfig.savefig(save + r'\fig_gainNVR_colorbar')
...
def plot_2DPpow_sweep_data_2D(h5filename, dataDir1, dataDirL, vna_states=('Signal', 'Idler'), nvr=True,
                              skip=1, cmap = ('hot', 'hot'), save=None, phase_save=False, i_backave=0):
    ...
    for i, dataDir in enumerate(dataDirs):
        ...
        plot_Ppow_sweep_data_2D(h5filename, dataDir=dataDir, vna_states=vna_states, nvr=nvr, skip=skip,
                                f_pump=p_freqs[i], colorMap=cmap, save=save, phase_save=phase_save, i_backave=i_backave)
...
def plot_Ppow_sweep_data_2D(h5filename, dataDir=None, vna_states=('Signal', 'Idler'), nvr=True, skip=1,
                            f_pump = 14.434e9, colorMap = ('rainbow', 'rainbow'), save=None, phase_save=False, i_backave=0):
    #i_backave: backgroundaveraging number of points on either ends of gain curve to average over for background, if 0 then no subtraction of background
...
#        ax_resI = ax_resS.twinx()
    ...
    for k in xrange(2):
        ...
        if i_backave:
            gain0 = (sum(gain[0, 0:i_backave]) + sum(gain[0,-25:i_backave])) / (2*i_backave)
            gain = gain - gain0
...
# extract resonator shift from phase of pump power sweep data - only works if overcoupled and phase=0 on resonance
...
def fit_gain_curve(freqs, a_logMag, display=True, memdataL=0):
    # freqs: array of frequency values to be fit (Hz)
    # a_logMag: S21 log mag
    # memDataL: log mag of memory trace, empty for no memtrace
    a_lin = np.power(10.0, 0.1 * (a_logMag-memdataL)) #switch to linear units
    ...
    if display:
        ...
        f, ax = plt.subplots()
        ax.plot(freqs*1e-9, a_logMag - memdataL, color='r',marker='o', fillstyle='none', ls='None', label='data')
        ax.plot(freqs*1e-9, fit_log, linewidth=4.0, color='b', label='f0=%.4f GHz, bw=%.1f MHz, G=%.1f dB'%(f0*1e-9,bw*1e-6,g_log))
...
#    p_off_lin = np.power(10, 0.1 * p_off) # (mW)    
    delta_p_log = p_on - p_off
    delta_p = np.power(10, 0.1*delta_p_log) # linear nvrp
    freqsM = freqs * 1e-6 #switch to MHz to help out the numerical fittings
...
#    p_on_lin_f0 = p_off_lin_f0 + popt[2] / popt[3] #TODO - wrong definition of NVR???
#    nvr_lin = p_on_lin_f0 / p_off_lin_f0
    nvr_lin = popt[2] / popt[3] + popt[1] # on resonace value of P_on / P_off
...
def get_gain_NVR(h5filename, dataDir=None, vna_states=('Signal','Idler'), nvr=True, recalc=False, display=True,  memDataDirs=None, memvna_fmt='SCOM'):
    ...
    if memDataDirs:
        for memDir in memDataDirs:
            data = load_sweepData(h5filename, memDir, vna_fmt=memvna_fmt)
            memdataL.append(complex_to_PLOG(data['a_complex'])[0])
        memdataL_S = memdataL[0]
        memdataL_I = memdataL[1]
    # fit only if recalc or if never been fit before
    ...
    if recalc or 'maxgain_S' not in vnakeys or 'maxgain_I' not in vnakeys:
        ...
        for i_pow, power in enumerate(vnadata['powers']):
            (f0_S[i_pow], maxgain_S[i_pow], bw_S[i_pow]) = fit_gain_curve(vnadata['freqs_S'], vnadata['gain_S'][i_pow,:], display=display, memdataL=memdataL_S)
            (f0_I[i_pow], maxgain_I[i_pow], bw_I[i_pow]) = fit_gain_curve(vnadata['freqs_I'], vnadata['gain_I'][i_pow,:], display=display, memdataL=memdataL_I)
            if nvr:
...
# plot Gain - NVR (dB) [SNR improvement] vs. Gain which results from a pump power sweep
def plot_GmNVR_G(h5filename, dataDir=None, vna_states=('Signal','Idler'), recalc=False, displayFit=True, save=None, memDataDirs=None, memvna_fmt='SCOM', title='SNR'):
    (powers, g_S, g_I, nvr_S, nvr_I) = get_gain_NVR(h5filename=h5filename, dataDir=dataDir, vna_states=vna_states, recalc=recalc, display=displayFit, memDataDirs=memDataDirs, memvna_fmt=memvna_fmt)
    ...
    axarr.plot(g_I, fit_I, color='b', linewidth=4.0, label=r'$T_{sys} / T_{JPC} = %.1f $'%(poptI[0]))
    width = 2.0    
...
# returns last save dataDir in h5file
def get_last_dataDir(h5filename):
    h5file = h5py.File(h5filename)
    day = h5file.keys()[-1]
    grp = h5file[day]
    t = grp.keys()[-1]
...
#    h5filename = r'Z:\Data\JPC_2016\2016-12-06 Cooldown\amj01.hdf5'
#    h5filename = r'Z:\Data\JPC_2016\2016-10-22 Cooldown\amj01.hdf5'
    h5filename = r'Z:\Data\JPC_2017\05-18-2017 Cooldown\jphf27.hdf5'
...
#    fridge = instruments['fridge']
    ...
    if False: # sweep flux
        data = flux_sweep(pnax, yoko, curr_start = -2.000e-3, curr_stop = 2.000e-3, curr_step=10e-6, averages=4, num_points=1601,
                   fridge=None,folder=None,display=False, atten=0,vna_fmt='POL', vna_sweep_type='LIN',
                   printlog=True, h5file=h5filename)
#        set_rtSwitch_S_SS()
#        vna.load_state('jphf23_S')
#        data = flux_sweep(vna, yoko, curr_start = -2.000e-3, curr_stop = 2.000e-3, curr_step=10e-6, averages=8, num_points=1601,
#                   fridge=None,folder=None,display=False, atten=0,vna_fmt='SCOM', vna_sweep_type='LIN',
#                   printlog=True, h5file=h5filename)  

    
    if False: # sweep flux and VNA power
        vna_pows = np.linspace(-80,-40,21)
        data = flux_VNAp_sweep(h5filename, vna, yoko, vna_pows=vna_pows, curr_start=1.600e-3, curr_stop=1.650e-3, curr_step=10e-6,
                    if_bandwidth=3000,num_points=1601, avgs_start=150, avgs_stop=2, display=False, atten=0, vna_fmt='SCOM')

    if False: # plot flux sweep
        define_phaseColorMap()
        colorMap_h5file(h5filename, dataDir='/170601/155901', colorMap=('hot','phase'))
    
    if False: # linecut
        dataDir = '/160620/111132' #'/160219/152324'
        lineCut_flux(h5filename, dataDir=dataDir, current=0.500e-3, verticalStack=True)

    if False: # dynamic range trace
        data = take_dynamic_range(vna, cw_freq=11.2942e9,pow_start=-80, pow_stop=-30,pow_step=0.1, avgs=200, revert=True, h5file=h5filename)
        ag3.set_rf_on(False)
    ...
    if False: #Pump power sweep
        data = take_Ppow_sweep(vna, ag3, spec=spec, pow_start=3.0, pow_stop=6.0, pow_step=0.25, averages=(100,50),
...
#        ag1.set_rf_on(False)
    ...
    if False: #2D pump power sweep
        data = take_2Dpump_sweep(vna, ag3, spec=None, pow_start=13.0, pow_stop=16.0, pow_step= 0.20, averages=(200,50),
                      freq_start=19.428e9, freq_stop=19.438e9, freq_step=0.5e6, num_points=1601, vna_states=('Signal','Idler'),
                        change_rtSwitch=(set_rtSwitch_S_SS,set_rtSwitch_S_II), display=True, atten=0, vna_fmt='PLOG', phase_save=True, h5file=h5filename)
#        ag1.set_rf_on(False)
    
    if False: # plot fitted flux sweep
        dataDir = '/170601/155901'
        (currents,Qc,Qi,Qtot,results) = flux_sweep_Qfit(h5filename, dataDir=dataDir, numFitPoints=400, kc=50e6, ki=1e6, display=False)
        plot_flux_Qfit(h5filename, dataDir=dataDir, vna_fmt='POL')
    
    if False: # power sweep Qfit
        dataDir = '/161031/140224'
        pow_fit = power_sweep_Qfit(h5filename, dataDir=dataDir, kc=40e6, ki=1e6, display=True, guessF=guessF_maxReal, save=True, numFitPoints=0)
        plot_power_Qfit(h5filename, dataDir=dataDir, vna_fmt='SCOM')
    ...
    if False: # plot SNR improvement
        ...
        plot_GmNVR_G(h5filename, dataDir='/161027/152705', vna_states=('Signal','Idler'), recalc=False, displayFit=True,
                     save=None, memDataDirs=memDataDirs, memvna_fmt='SCOM')
                     #save=r'Z:\Talks\Nick\APS 2016\fig_constNoiseModel2.png'
        
    if False: #plot pump power sweep traces
        memDataDirs = ['/161027/111646', '/161027/140859']#/161027/174724
        plot_Ppow_sweep_data(h5filename, dataDir='/161027/200731', vna_states=('Signal', 'Idler'), nvr=True, skip=2,
                             cutTop=4, memDataDirs=memDataDirs, memvna_fmt='SCOM',
                             save=r'C:\Users\JPC-Acquisition\Documents\nick\Kerr-Free3Wave_Paper\fig3',  atten_p=-62.5)
                    
    if False: #plot pump power sweep, 2D plot
        plot_Ppow_sweep_data_2D(h5filename, dataDir='/161020/141355', vna_states=('Signal', 'Idler'), nvr=True, skip=1,
                                f_pump = 14.2068e9, colorMap = ('hot', 'hot'), save=None, phase_save=True, i_backave=25)
    
    if False: # plot 2D pump sweep, 2D plot
        plot_2DPpow_sweep_data_2D(h5filename, '/170422/002038', '/170422/091614', vna_states=('Signal', 'Idler'), nvr=False,
                              skip=2, cmap = ('hot', 'hot'), save=None, phase_save=False, i_backave=25)
