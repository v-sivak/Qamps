...
#
def complex_fit(f, xData, yData, p0, weights=None):
    if np.isscalar(p0):
        p0 = np.array([p0])
...
def fit_const_noise_model(g_lin, g_nvr_lin):
    A = g_nvr_lin[-1]
    popt, pcov = curve_fit(const_noise_model, g_lin, g_nvr_lin,(A))
    return popt
# G/NVR = (1/A + 1/x)^-1 where x = Gain and A = T_sys / (2T_Q)
def const_noise_model(x, A):
    denominator = 1.0/A + 1.0/x
    return 1/denominator

def fit_resonance(peak,bg):
    freq = peak[0]
    gammadB = peak[1]-bg[1]
    gamma = pow(10.0,0.1*gammadB)
    print fit_lorentzian(freq,gamma)
...
def gaussian_sum(t, A1, sigma1,t01, A2, sigma2,t02):
    y=A1*exp(-((t-t01)/sigma1)**2/2)+A2*exp(-((t-t02)/sigma2)**2/2)
    return y
...
def fit_hist(data_postselect_raw, plot_hist=True, logplot=True):
    numbins=100
    ...
    histi_re, bin_edges = np.histogram(np.real(data_postselect_raw), bins=numbins, density=True)
    histi_im, bin_edges = np.histogram(np.imag(data_postselect_raw), bins=numbins, density=True)
    hist2D, xedges, yedges = np.histogram2d(np.imag(data_postselect_raw), np.real(data_postselect_raw), [numbins-1, numbins-1])#, range=[[xmin, xmax], [xmin, xmax]])
    ...
    sigma2=sigma1
    popt, pcov = scipy.optimize.curve_fit(gaussian_sum, x, y, (A1, sigma1,t01, A2, sigma2,t02),maxfev=100000)
    ...
    if plot_hist:
        ...
        for i in range(np.size(bin_edges)-1):
            if bin_edges[i]<threshold:
                Pgth=Pgth+histi_re[i]*(bin_edges[1]-bin_edges[0])
        ...
        if logplot:
            ax[0][0].pcolor(yedges, xedges, np.log(1+hist2D), cmap='gist_yarg')
        else:
            ax[0][0].pcolor(yedges, xedges, hist2D, cmap='gist_yarg')
        ax[0][0].axis('equal')
        ax[0][0].axis('tight')
...
def analyze_reflection(freq, ropows, data, f_0=7.226e9, kc=380e3, ki=380e3, a_in=None, T=-400e-9, display=True):
    if np.size(ropows)==1:
        data=np.array([data])
    ...
    for ii in range(np.size(ropows)):
        ...
        if display:
            ...
            ax11.axis('equal')
            ax12.plot(freqfit*1e-9, np.angle(fitfunc, deg=True))
            ax12.plot(freq*1e-9, np.angle(data[ii], deg=True), 'o')
            ax13.plot(freqfit*1e-9, np.abs(fitfunc))
            ax13.plot(freq*1e-9, np.abs(data[ii]), 'o')
            ...
            ax21.plot(np.real(fitfunc_mod), np.imag(fitfunc_mod), linewidth=4.0,color='b')
            ax21.plot(np.real(data_mod), np.imag(data_mod),marker='o',color='r', fillstyle='none',alpha=0.1)
            ax21.axis('equal')
            ax22.plot(freqfit*1e-9, np.angle(fitfunc_mod, deg=True), linewidth=4.0, color='b') # can alter phase of fitfunc by *np.exp(1j*np.pi)
            ax22.plot(freq*1e-9, np.angle(data_mod, deg=True), marker='o',color='r', fillstyle='none',alpha=0.1)
            ax23.plot(freqfit*1e-9, np.abs(fitfunc_mod), linewidth=4.0, color='b')
            ax23.plot(freq*1e-9, np.abs(data_mod), marker='o', color='r', fillstyle='none',alpha=0.1)
            ax21.set_xlabel('Re{a_out / a_in}')
            ax21.set_ylabel('Im{a_out / a_in}')
            ax22.set_ylabel('Phase (deg)')
            ax23.set_xlabel('Frequency (GHz)')
            ax23.set_ylabel('|a_out / a_in|')
...
def fit_complex_a_out(f, a_out, f_0=7.226e9, kc=500e3, ki=500e3, a_in=None, T=70e-9):
    def aux(f, f_0, kc, ki, re_a_in, im_a_in, T): #to take out electrical delay, remove T as an argument
        return complex_a_out(f, f_0, kc, ki, re_a_in + 1j*im_a_in, T)
    if a_in is None:
        a_in = a_out[0]
#    popt = complex_fit(aux, f, a_out, (f_0, kc, ki, np.real(a_in), np.imag(a_in), T)) #can take out T from fitting parameters
    popt = complex_fit_iterative(aux, f, a_out, (f_0, kc, ki, np.real(a_in), np.imag(a_in), T))
    
    return [popt[0], popt[1], popt[2], popt[3] + 1j*popt[4], popt[5]] #to take out electical delay as a parameter, popt[5] -> 0
...
def complex_fit_iterative(f, xData, yData, p0, weights=None, bounds=()):
    if np.isscalar(p0):
        p0 = np.array([p0])
    ...
    popt, pcov, infodict, mesg, ier = leastsq(residuals, p0, args=(xData, yData), full_output=True)
    perr_best = np.sqrt(np.diag(pcov))[0]
    p0_best = p0
        
    for f_0 in [ xData[len(xData)/6], xData[2*len(xData)/6], xData[3*len(xData)/6], xData[4*len(xData)/6], xData[5*len(xData)/6] ]:
        p0_new = (f_0, p0[1], p0[2], p0[3], p0[4], p0[5]) 
        popt, pcov, infodict, mesg, ier = leastsq(residuals, p0_new, args=(xData, yData), full_output=True)
        perr = np.sqrt(np.diag(pcov))[0]
        if perr < perr_best:
            p0_best = p0_new
    popt, pcov, infodict, mesg, ier = leastsq(residuals, p0_best, args=(xData, yData), full_output=True)            
