...
if __name__ == '__main__':
    ...
    figIMD, axes = plt.subplots(1, 2, sharex='col', sharey='row', figsize=(7, 2.0))
...
#    ax.plot(pump_detuning*1e-6, line_19dB, linestyle='none', markersize=1.5, marker='o',color=palette(2))
    ...
    ax.set_ylim(-123, -95)  #-123
    ax.set_ylim(11, 36)

    # Plot phase transition figure

    fig2, axes = plt.subplots(2, 1, sharex='col', figsize=(3.375, 3))#, sharex='col', sharey='row', figsize=(7, 2.0), gridspec_kw = {'width_ratios':[3, 3, 1]})
    ax = axes[0]
    ax.set_xlabel(r'Input power (dBm)')
    ax.set_ylabel(r'Gain (dB)')

    cmap = plt.get_cmap('coolwarm_r')
    norm = mpl.colors.Normalize(vmin = pump_detuning2[60]*1e-6, vmax = pump_detuning2[0]*1e-6)
    

    for i in range(60):#range(np.shape(gain_array)[0]):
...
#        ax.plot([signal_powers[0][indmax]],[gain_array[i][indmax]],color='black',zorder=2,marker='.',markersize=3)
    ...
    cb.set_label('Pump detuning $\Delta/2\pi$ (MHz)', rotation=90) #APS: fontsize='x-large', labelpad=2
    ...
    for i, pump_detuning in enumerate(pump_detuning2[:60]):
        ...
        G = 10**(np.linspace(12,10*np.log(G_max),2001)/10)
        ...
        G = G[ind]
        ax.plot(P1, 10*np.log10(G), color = color)
        ...
        G = G[ind]
        ax.plot(P2, 10*np.log10(G), color = color)
